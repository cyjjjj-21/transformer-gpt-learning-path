...
- [03-论文阅读索引-Transformer-GPT](notes/03-论文阅读索引-Transformer-GPT.md)
  这是论文阅读地图，把 `Seq2Seq -> Attention -> Transformer -> GPT -> Scaling -> 后训练` 串成一条连续演化线。

- [examples/](examples/README.md)
  这是本地 demo 目录，包含环境自检、tokenizer、最小 GPT、轻量训练和采样对照实验。

- `scripts/`
  这里放了一些轻量辅助脚本，例如环境自检、实验脚手架创建和 Mermaid 图重渲染。

- `assets/`
  这里放学习路径图、论文阅读地图和本地 demo 工作流图，同时保留 `.mmd` 和 `.png` 两种形式。
...
![学习路径图](assets/learning-roadmap.png)
...
如果你使用 Obsidian，可以把整个仓库作为一个 vault 或者子目录导入，配合 `01`、`02`、`03` 和 `examples/` 持续记录学习和实验进度。
